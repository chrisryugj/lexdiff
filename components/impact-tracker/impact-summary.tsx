        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex items-center gap-2 sm:gap-3 mb-4 flex-wrap">
              ...
              <span className="text-sm font-semibold text-[#1a2b4c] dark:text-white">
                총 {summary.totalChanges}건
              </span>

              <div className="flex gap-1.5 ml-auto">
                {summary.bySeverity.critical > 0 && (
                  <span className="text-[11px] font-semibold text-red-600 bg-red-500/10 dark:bg-red-500/20 dark:text-red-400 px-2 py-0.5 rounded-full">
                    긴급 {summary.bySeverity.critical}
                  </span>
                )}
                {summary.bySeverity.review > 0 && (
                  <span className="text-[11px] font-semibold text-amber-700 bg-amber-500/10 dark:bg-amber-500/20 dark:text-amber-400 px-2 py-0.5 rounded-full">
                    검토 {summary.bySeverity.review}
                  </span>
                )}
                {summary.bySeverity.info > 0 && (
                  <span className="text-[11px] font-semibold text-emerald-700 bg-emerald-500/10 dark:bg-emerald-500/20 dark:text-emerald-400 px-2 py-0.5 rounded-full">
            ...
            {summary.aiSummary && (
              <div>
                <button
                  ...
                  className="text-sm text-[#1a2b4c] dark:text-[#e2a85d] hover:opacity-80 flex items-center gap-1.5 font-medium transition-opacity"
                >
                ...
                {expanded && (
                  <div className="mt-3 text-sm text-gray-700 dark:text-gray-300 leading-relaxed prose prose-sm dark:prose-invert max-w-none prose-headings:text-base prose-headings:font-semibold prose-headings:mt-3 prose-headings:mb-1 prose-p:my-1 prose-ul:my-1 prose-li:my-0.5 bg-[#faf9f7] dark:bg-gray-800/30 rounded-lg p-4 border border-gray-100 dark:border-gray-800">
