...
// 질문 종결어미 패턴
// 한국어 조사(은/는/이/가/을/를/에/에서/의/도/만/까지/으로/로) 허용
const KO_PARTICLES = '(?:은|는|이|가|을|를|에서|에|의|도|만|까지|으로|로)?'
export const QUESTION_ENDINGS = new RegExp(
  `[?？]$|인가요?$|인지요?$|될까요?$|되나요?$|습니까?$|니까?$` +
  `|알려줘|설명해줘|가르쳐줘|말해줘|찾아줘|보여줘|궁금|뭐야|뭐지|뭔지|뭘까$` +
  `|(?:방법|절차|요건|조건|기준|대상|차이|비교|종류|유형|범위|내용|정의|의미|효력|처벌|벌칙|위반|제재)${KO_PARTICLES}$`
)
