...
**대규모 코드베이스 분석** (필요시에만):
1. **분할 읽기**: offset/limit 파라미터로 큰 파일 나눠 읽기
2. **병렬 도구 호출**: 독립적인 Glob/Grep은 한 번에 호출
3. **Sub Agent**: 복잡한 아키텍처 분석에만 사용 (단순 작업은 직접 처리)
...
**코드 변경 전** (복잡도에 따라 선택):
- 🟢 **단순 버그 수정**: 해당 파일만 읽기
- 🟡 **기능 추가/수정**: CLAUDE.md Quick Reference + 해당 파일
- 🔴 **아키텍처 변경/새 모듈**: 전체 문서 체인 (CLAUDE.md → agent.md → 상세 문서 → 코드)

**원칙**: 변경할 파일은 반드시 읽기 (관련 없는 문서는 스킵 가능)
...
### 코드베이스 탐색 (복잡도별 선택)
- 🟢 **단순 검색** ("X 파일 어디?", "Y 함수 찾아줘"): Glob/Grep 직접 사용
- 🔴 **복잡한 탐색** ("X가 어떻게 작동?", "Y 아키텍처 설명"): Task (subagent_type=Explore)
...
**버전**: 2.8 | **업데이트**: 2025-12-04 | **줄 수**: ~245 | **변경**: 토큰 최적화 (복잡도별 차등 접근)
