...
**Bug:** Empty lawName accepted by API

**RED**
```typescript
test('rejects empty lawName', async () => {
  const response = await GET(mockRequest({ lawName: '' }))
...
```bash
$ npm test
FAIL: expected status 400, got 200
```
...
**REFACTOR**
Extract validation helper for multiple routes if needed.
