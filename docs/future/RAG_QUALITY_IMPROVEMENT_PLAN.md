...
**파일**: `lib/file-search-client.ts`
**위치**: systemInstruction 정의 부분

> ⚠️ **중요**: 아래 프롬프트는 `lib/ai-answer-processor.ts`의 HTML 변환 로직과 완전히 호환됩니다.
> 기존 로직이 인식하는 패턴만 사용합니다:
> - 주요 섹션: `📋 핵심 요약`, `📄 상세 내용`, `💡 추가 참고`, `🔗 관련 법령`
> - 핵심 요약 하위: `✅`, `📌`, `🔔` 이모지
> - 상세 내용 하위: `⚖️ 조문 발췌`, `📖 핵심 해석`, `📝 실무 적용`, `🔴 조건·예외`

```typescript
// 질문 유형별 프롬프트 템플릿 (ai-answer-processor.ts 호환)
