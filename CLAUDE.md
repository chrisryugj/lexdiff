...
2. **Key API Routes**:
   ...
   - `/api/summarize`: Generate AI summary using Gemini
...
- **JSON**: Current law text (`/api/eflaw`) → parsed by `parseLawJSON()` in `app/page.tsx`
- **XML**: Old/new comparison (`/api/oldnew`), search results, ordinances
  - Use DOMParser to parse XML
  - Extract specific nodes with `querySelector()`/`querySelectorAll()`
  - Example parsers: `lib/law-search-parser.ts`, `lib/ordin-parser.ts`, `lib/revision-parser.ts`
