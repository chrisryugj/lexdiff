...
// ─── 범용 출력 형식 (웹 방식 재설계 — relay 경로) ───
//
// SPECIALIST_INSTRUCTIONS[8종]은 queryType 오분류 시 부적합 구조를 강제해 엣지케이스를
// 만든다(예: definition 구조의 "## 조문 원문" 강제 → 도구예산과 충돌해 "원문 미확인" 사과 노출).
// 강한 모델(relay Sonnet)은 출력 계약만 주고 중간 구성은 내용에 맞게 자율 선택하게 한다.
// 파서 하드 제약(유지 필수): 첫 ## 헤딩 시작 · '## 근거 법령' 섹션 · 「법령명」 제N조 · [별표 N].
const UNIVERSAL_INSTRUCTIONS = `첫 줄부터 ## 헤딩으로 시작. 대항목은 ## 헤딩, 소항목은 ### 헤딩:
## 결론
질문에 대한 직접적인 답부터 (가능 여부/금액/핵심 차이 등 + 짧은 이유)
## 주요 내용
근거 상세 — 내용에 맞는 형식을 스스로 선택: 절차·단계는 ### 소제목, 금액·기준·비교는 | 표 |, 요건은 목록.
필요하면 ## 주의사항, ## 신청 방법, ## 비교표 등 대항목을 자유롭게 추가 (구조를 억지로 채우지 말 것)
## 근거 법령
본문에서 인용한 모든 법령을 「법령명」 제N조 (시행 YYYY-MM-DD) 형식으로 나열`
 ...
 * 별표 공용원칙(금액=별표, 번호 불확실 시 목록 먼저)만 조회 힌트로 유지 —
 * 이건 전략이 아니라 법제처 데이터 구조에 대한 사실 지식이라 모델이 알 수 없음.
 */
export function buildStaticSystemPrompt(isGemini?: boolean, opts?: PromptBuildOptions): string {
...
- 도구 호출 없이 생성된 답변은 검증 불가능한 환각으로 간주되어 전면 거부된다.`
  ...
  const toolStrategyBlock = autonomous
    ? `## 도구 사용
- 어떤 도구를 어떤 순서로 몇 번 쓸지는 질문에 맞게 **스스로 판단**하되, 필요 최소한으로 (속도).
- **금액·수수료·요금·과태료·과징금·단가·기준·한도 질의**(질의 유형과 무관): 구체적 액수는 조문 본문이 아니라 [별표]에 표로 규정된 경우가 대부분임. 관련 시행령/시행규칙의 별표를 get_annexes로 조회해 실제 금액을 확인할 것. **별표 번호가 불확실하면 lawName 만으로 호출해 별표 목록을 먼저 받은 뒤, 목록에서 해당 별표를 골라 재조회**할 것 (번호를 추측해 헛조회하지 말 것). 조회한 별표가 질의와 무관하면 목록의 다른 별표를 확인하고, 그래도 없으면 소관 고시(공단/기관) 가능성을 명시.`
    : `## 도구 사용 (우선순위)
...
## 🔴 현행 법령 기준 (시행일자 확인 필수)
- 답변은 **현행 법령** 기준. 도구 결과의 시행일자·현행성 라벨([현행]/[연혁]/⚠️ 표시)을 반드시 확인할 것.
- 도구 결과에 "연혁", "과거버전", "시행 예정", "구 법령명" 표시가 있으면 그 사실을 답변에 명시하고, 현행 기준이 필요하면 [현행] MST로 재조회할 것.
- 도구 결과의 시행일자가 [답변 지침]의 오늘 날짜보다 **미래**이면 그 조문은 아직 효력이 없는 **시행예정본**임 — 현행으로 단정하지 말고 시행예정(시행일 명시)임을 밝힐 것. (라벨이 없어도 날짜 비교로 판단)
- 판례가 적용한 조문은 판결 당시의 **구(舊) 조문**일 수 있음. 판례를 근거로 조문을 인용할 때는 "구 「법령명」(YYYY. M. D. 개정 전) 제N조" 형식으로 현행 조문과 구분해 표기하고, 현행과 내용이 다를 수 있음을 밝힐 것.
- 법령은 개정·분법·폐지가 잦음 (예: 구 「화재예방, 소방시설 설치ㆍ유지 및 안전관리에 관한 법률」은 「소방시설 설치 및 관리에 관한 법률」과 「화재의 예방 및 안전관리에 관한 법률」로 분법됨). 학습 데이터의 옛 법령명·조문번호·기준치를 절대 사용하지 말 것.
- 답변 마지막 "## 근거 법령"에 인용 법령의 **시행일자를 함께 표기**할 것 (예: 「소방시설 설치 및 관리에 관한 법률」 제12조 (시행 2024-12-01)).
- 개정 전후 비교가 필요한 질문에만 ${autonomous ? '신구대조·개정이력을 조회' : 'chain_amendment_track 또는 compare_old_new 사용'}. 그 외에는 항상 현행 본문 기준.
...
/** 프롬프트 빌드 옵션 (웹 방식 재설계 — relay 경로 전용, 미지정 시 기존 동작) */
export interface PromptBuildOptions {
  /** SPECIALIST[queryType] 대신 범용 출력 형식 사용 */
...
export function buildDynamicHeader(
  ...
  // 조회 전략 자율 모드: 도메인/결정문/벌칙조 힌트는 전부 "무엇을 조회하라"는 지시라 제거.
  // (약칭 해석은 조회 전략이 아니라 법제처 API 함정 회피용 사실 힌트 → 유지)
  const autonomous = opts?.autonomousTools === true

  const domain = query && !autonomous ? detectDomain(query) : 'general'
  const domainHint = DOMAIN_TOOL_HINTS[domain] || ''

  const consequenceHint = queryType === 'consequence' && !autonomous
  ...
  const domainBlock = domainHint && !autonomous ? `\n## 질의 도메인 힌트\n${domainHint}` : ''

  // 특수 결정문 도메인 감지 → LLM 에 search_decisions 도메인 명시적 지시
  const decisionHits = query && !autonomous ? detectDecisionDomains(query) : []
  ...
  // 오늘 날짜(KST) — static prompt에 넣으면 Gemini context cache가 매일 깨지므로
  // 동적 헤더에만 주입. LLM이 시행일자/개정 여부를 판단할 기준점.
  const todayKST = new Intl.DateTimeFormat('sv-SE', { timeZone: 'Asia/Seoul' }).format(new Date())

  // 자율 모드에선 복잡도 라인의 "도구 예산 준수"도 의미 없음(예산 지시 자체가 없음) → 분량 힌트만.
  const complexityLine = autonomous
    ? `- 분량: ${LENGTH_HINT[complexity]}`
    : `- 복잡도: **${complexity}** (도구 예산 준수)\n- 분량: ${LENGTH_HINT[complexity]}`

  const structureBlock = opts?.universalFormat
...
- 오늘 날짜: **${todayKST}** (KST)
${complexityLine}
${structureBlock}${consequenceHint}${domainBlock}${decisionBlock}${aliasBlock}
...
export function buildSystemPrompt(
  ...
  return `${buildStaticSystemPrompt(isGemini, opts)}\n\n${buildDynamicHeader(complexity, queryType, query, opts)}`
}
