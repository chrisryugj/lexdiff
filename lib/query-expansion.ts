...
const TOPIC_CLUSTERS: Record<string, string[]> = {
  ...
  // 세금/재정 클러스터
  "세금": ["조세", "국세", "지방세", "감면", "비과세", "면세", "과세", "납세", "세입", "세수", "세무"],
  ...
  "지방세": ["취득세", "재산세", "자동차세", "주민세", "등록면허세", "지방교육세", "레저세"],
...
export function expandQuery(query: string, excludeRegion = true): {
  ...
  // 지역명 패턴
  const regionPattern = /((?:서울특별시|부산광역시|대구광역시|인천광역시|광주광역시|대전광역시|울산광역시|세종특별자치시|경기도|강원특별자치도|충청북도|충청남도|전라북도|전북특별자치도|전라남도|경상북도|경상남도|제주특별자치도)\s*)?([가-힣]+[시군구])/
  const regionMatch = cleaned.match(regionPattern)
  ...
  if (region && keywords.length > 0) {
    ...
    for (const kw of keywords) {
      ...
      for (const synonym of expansion.expanded) {
        if (synonym !== kw) {
          addStrategy(`${region} ${synonym}`, 20, undefined, `동의어: "${kw}" → "${synonym}"`)
        }
      }
    }
    ...
    addStrategy(region, 200, filterArr, `필살기: "${region}" 전체 → [${filterArr.slice(0, 8).join(',')}${filterArr.length > 8 ? '...' : ''}] 필터`)
