...
const PRECEDENT_PATTERNS = [
  ...
  /(대법원|서울고등법원|서울고법|부산고등법원|부산고법|대구고등법원|대구고법|광주고등법원|광주고법|대전고등법원|대전고법|서울중앙지법|서울동부지법|서울남부지법|서울북부지법|서울서부지법|인천지법|수원지법|부산지법|대구지법|광주지법|대전지법|울산지법|창원지법|의정부지법|춘천지법|청주지법|전주지법|제주지법)\s*\d{4}(도|나|가|다|마|바|자|카|타|파|하|두|구|누|부|머|사|로|고단|고합|가합|가단|노|초|추)\s*\d+/,
  // 법원명 + 연도만 (예: "대법원 2025")
  /(대법원|서울고등법원|서울고법|부산고등법원|부산고법|대구고등법원|대구고법|광주고등법원|광주고법|대전고등법원|대전고법)\s*\d{4}/,
  // 사건번호만 (연도 + 사건유형 + 번호)
  /\d{4}(도|나|가|다|마|바|자|카|타|파|하|두|구|누|부|머|사|로|고단|고합|가합|가단|노|초|추)\s*\d+/,
  // 판례/판결/결정 + 연도
  /(판례|판결|결정).*\d{4}년/,
  // 법원 종류 + 판결/결정
...
const ORDINANCE_PATTERNS = [
  ...
  // ✅ 서울 25개 구 + 키워드 → 조례 후보 (조례 키워드 없어도 인식)
  new RegExp(`(${SEOUL_DISTRICTS.join('|')})\\s+[가-힣]+`)
]

// 시행규칙/시행령 패턴 (법령으로 분류되어야 함)
const LAW_ENFORCEMENT_PATTERN = /시행령|시행규칙/
...
// 질문 종결어미 패턴
// "방법", "절차", "요건" 등으로 끝나는 것도 질문으로 인식
const QUESTION_ENDINGS = /[?？]$|인가요?$|인지요?$|될까요?$|되나요?$|습니까?$|니까?$|알려줘|설명해줘|가르쳐줘|말해줘|찾아줘|보여줘|궁금|뭐야|뭐지|뭔지|뭘까$|방법$|절차$|요건$|조건$|기준$|대상$/
