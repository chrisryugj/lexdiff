...
function parseArticleComponents(input: string): ArticleComponents {
  ...
  if (/^\d{4,6}$/.test(sanitized)) {
    const padded = sanitized.padStart(6, "0")
    const articleSlice = padded.slice(0, 4)
    const branchSlice = padded.slice(4, 6)

    const articleNumber = Number.parseInt(articleSlice, 10)
    const branchNumber = Number.parseInt(branchSlice, 10)

    if (!Number.isNaN(articleNumber) && !Number.isNaN(branchNumber)) {
      debugLogger.debug("숫자형 JO 감지", { input, sanitized, articleNumber, branchNumber })
...
export function parseSearchQuery(query: string): ParsedSearchQuery {
  ...
  if (match && match.index !== undefined) {
    ...
    const parsed: ParsedSearchQuery = {
      ...
      jo,
      matchedAlias: lawNameResolution.matchedAlias || undefined,
      aliasAlternatives: lawNameResolution.alternatives,
    }
