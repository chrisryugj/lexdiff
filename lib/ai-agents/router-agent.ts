...
- definition: 개념/정의/해석 질문 ("~이란?", "~의 정의", "~의 의미", "~에 대해 알려줘", "~자세히", "해석")
...
- civil-service: 공무원법 (승진, 징계, 휴직, 공무원연금, 소청, 복무, 복무조례, 근무)
...
응답:
{
  ...
  "extractedLaws": [],
  "extractedArticles": [],
  "extractedEntities": ["FTA"],
  "intent": "FTA(자유무역협정)의 개념과 의미를 알고자 함",
  "subQuestions": null,
  "confidence": 0.92,
  "reasoning": "'자세히 알려줘'는 definition. FTA는 customs 도메인.",
  "searchOptimization": {
    "optimizedQuery": "FTA 자유무역협정 정의 개념",
    "searchKeywords": ["FTA", "자유무역협정", "정의"],
    "relatedTerms": ["협정관세", "원산지", "원산지증명서", "특혜관세"],
    "synonyms": ["자유무역협정", "Free Trade Agreement"],
    "searchHints": {
      "targetSection": null,
      "articleRange": null,
      "keyProvisions": ["FTA", "자유무역협정"]
    },
    "strategy": "semantic"
  }
}
...
export async function analyzeQuery(query: string): Promise<RouterAnalysis> {
  // ✅ 임시: 메인 키 사용 (GEMINI_ROUTER_API_KEY 쿼터 초과)
  const apiKey = process.env.GEMINI_API_KEY || process.env.GEMINI_ROUTER_API_KEY
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY or GEMINI_ROUTER_API_KEY is required')
  }
