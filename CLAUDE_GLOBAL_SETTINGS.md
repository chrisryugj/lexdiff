...
1. **불명확한 요구사항 확인**:
   - 모호하거나 정보가 부족한 부분이 있으면 **절대 임의로 결정하지 말 것**
   - "확인이 필요한 질문 리스트"를 우선순위에 따라 정리
   - 각 질문에 대한 **추천 안(Best Practice)**을 함께 제시
...
3. **사용자 답변 후 진행**:
   - 모든 불명확한 사항이 해결된 후에만 작업 시작
   - 필요시 agent.md 참조하여 적절한 에이전트 활용
...
- ❌ 서버 재시작 직접 실행 금지
  - `npm run dev` 재실행
  - `kill` 명령으로 프로세스 종료 후 재시작
  - 개발 서버 stop/start
- ❌ 브라우저 캐시 새로고침 제안 금지
  - "브라우저 캐시를 지우세요"
  - "하드 리프레시 (Ctrl+Shift+R) 하세요"
  - "시크릿 모드로 여세요"

**대신 해야 할 것**:
- ✅ 서버 재시작이 필요하면: **"서버를 재시작해주세요"** (사용자에게 요청)
- ✅ 변경사항 확인이 필요하면: **"변경사항을 확인해주세요"** (구체적 방법 없이)

**이유**:
- 서버 재시작은 사용자가 직접 제어해야 함
- 브라우저 캐시는 사용자 환경에 따라 다름
- 불필요한 기술적 지시보다 간단한 요청이 더 효과적
